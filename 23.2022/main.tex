...
\angle H(f)&=-2\pi fT_d
\end{align}
Here, $\angle H(f)$ is directly propotional to frequency and time delay.$\angle H(f)$ vs $f$ gives a straight line with slope $-2\pi T_d$ The system will acts as pure delay sytem. Now, if we take $f^2$ as frequency, the system doesn't act as pure delay syatem because $\angle H(f)$ is propotional to $f^2$ but not $f$. Here,$\angle H(f)$ vs $f$ gives parabola.\\
...
\end{align}
From \eqref{eq:2} and \figref{fig:11}, $|H(f)|$ is same. Statement 1 is correct.
