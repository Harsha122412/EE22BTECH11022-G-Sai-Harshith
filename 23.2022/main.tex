...
\label{eq:7}
\end{align}
$\angle H(f)$ is directly propotional to frequency and time delay. The system will acts as pure delay sytem. Now, if we take $f^2$ as frequency, the system doesn't act as pure delay system.\\
Example : Consider 
\begin{align}
H(f)=e^{-2\pi f^2jT_d}
\end{align}
...
S_X(f)&=|X(f)|^2\\
S_Y(f)&=|Y(f)|^2\\
S_Y(f)&=\frac{|Y(f)|^2}{|X(f)|^2}S_X(f)\\
&=\left|\frac{Y(f)}{X(f)}\right|^2S_X(f)\\
&=|H(f)|^2S_X(f)
\end{align}
