...
\end{align}
From \eqref{eq:2} and \figref{fig:11}, $|H(f)|$ is same.But from \eqref{eq:7}, $\angle H(f)$ is directly propotional to frequency and time delay.$\angle H(f)$ vs $f$ gives a straight line with slope $-2\pi T_d$ The system will acts as pure delay sytem. Now, if we take $f^2$ as frequency, the system doesn't act as pure delay syatem because $\angle H(f)$ is propotional to $f^2$ but not $f$. Here,$\angle H(f)$ vs $f$ gives parabola. Therfore system doesn't necessarily be pure delay even $|H(f)|$ is same \\
We have Statement 1 is incorrect.
